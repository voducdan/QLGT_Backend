...
namespace UnitTests
{
    public class KhachHangTests
    {
        ...
        [TestCase(1)]
        [TestCase(2)]
        public void GetCustomerTest_WithValidMaKH_ReturnList(int MaKH)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
        ...
        [TestCase(100)]
        [TestCase(222)]
        public void GetCustomerTest_WithInvalidMaKH_ReturnNull(int MaKH)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
        ...
        [TestCase("123441211")]
        [TestCase("123441213")]
        public void AddCustomerTest_WithValid_ReturnSuccessful(string CMND)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
        ...
        [TestCase("262626226")]
        [TestCase("262626227")]
        public void AddCustomerTest_WithExistedCMND_ReturnFail(string CMND)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
            using (var context = new QLGTDBContext(options))
            {
                ...
                command.HOAT_DONG = test_cus.HOAT_DONG;
                var result = cus_controller.Create(command) as OkObjectResult;
                Assert.AreEqual("{ success = False, error = CMND is exist }", result.Value.ToString());
            }
        }
        [Test]
        [TestCase(2)]
        [TestCase(3)]
        public void UpdateCustomerTest_WithValidMaKH_ReturnSuccessfulRepon(int MaKH)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
        ...
        [TestCase(3)]
        [TestCase(8)]
        public void UpdateCustomerTest_WithInvalidMaKH_ReturnFailedRepon(int MaKH)
        {
            var options = new DbContextOptionsBuilder<QLGTDBContext>()
                .UseInMemoryDatabase("KhachHangList")
                .Options;
